...
### 8. Add Navigation Link and Route

Edit `src/app.rs` to configure navigation and routing.

**Step 1: Import the generated route component**

The `mosaic_playground_macro::generate_component_pages!()` macro generates route components, but you must manually import them. Add the route to the imports at the top of `app.rs` (around line 9-11):

```rust
// Generated by macro
use crate::{
    AccordionRoute, BadgeRoute, /* ... */, ComponentNameRoute, /* ... */
};
```
...
Add the route in the Routes section (around line 258-260):

```rust
<Route path=StaticSegment("component-name") view=ComponentNameRoute />
```

**Important**: The route component (e.g., `ComponentNameRoute`) is auto-generated by the macro based on the component name in `component.toml`, but you must manually add it to the imports in `app.rs` for it to be accessible.
...
The `mosaic_playground_macro::generate_component_pages!()` macro in `src/lib.rs`:
...
- [ ] Import generated route component in `src/app.rs` (e.g., `ComponentNameRoute`)
- [ ] Add navigation link in `src/app.rs`
- [ ] Add route definition in `src/app.rs`
...
### Route Not Found / `ComponentNameRoute` not in scope

**Most Common Issue**: The generated route component is not imported in `app.rs`.

The macro generates route components (e.g., `LinkRoute`, `ButtonRoute`), but you must manually add them to the imports at the top of `app.rs`:

```rust
// Generated by macro
use crate::{
    AccordionRoute, BadgeRoute, /* ... */, YourComponentRoute, /* ... */
};
```

**Other possible causes**:
- Verify the component name in TOML matches the expected pattern
- Check that `mosaic_playground_macro::generate_component_pages!()` is called in `src/lib.rs`
- Rebuild the project completely: `cargo clean && cargo leptos watch`
- Clean and rebuild the macro: `rm -rf target/debug/deps/*mosaic_playground_macro* && cargo build`
