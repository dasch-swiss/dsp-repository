...
- **dsp-cli** (`dsp-cli/`) — the platform's command-line client: talks to a DSP server using researcher vocabulary (data-models, resource-types, fields) instead of DSP-API's raw RDF/JSON-LD surface (ADR-0002). No area crate ever depends on dsp-cli; it depends on their public HTTP surface only, the same integration every other external client uses. → [`dsp-cli/CONTEXT.md`](dsp-cli/CONTEXT.md).
...
- **"Representation"** means three things: DPE's *machine-readable representation* (one serialization of a published object's metadata — the schema.org JSON-LD, the DataCite XML, the Dublin Core record; `shared-fair` writes one per format off a single resolved graph, and FAIR Signposting's `describedby` links point at them), the Archive Area's `Representation` entity (a preservation-grade bundle of Preservation Files plus their metadata, ARK-identified and versioned, pinned by Resource versions), and dsp-cli's Representation (whether a VRE resource-type carries a binary asset, and which kind — `still-image`, `moving-image`, `audio`, `document`, `archive`, or `text`; see `dsp-cli/CONTEXT.md`). Resolution: "machine-readable representation" for the serialization, the archive's is never qualified inside `areas/archive/`, and "dsp-cli's Representation" for the VRE's file-bearing resource-type facet; neither is ever just "the representation" in prose that could mean another.
...
- **"Project"** means three things: `ProjectRaw` (the contract, the file as read), `dpe_core::Project` (DPE's view model, whose `From` impls are lossy on `url` and `clusters`), and dsp-cli's Project, the VRE project record as DSP-API serves it (`dsp-cli/CONTEXT.md`). Resolution: the editor's path is `ProjectRaw` → draft → `ProjectRaw` and never touches the view model; say "view model" when you mean DPE's. The Shortcode identifies the same research project across the VRE and the Repository, but the records differ — a live administrative record in the VRE, an archived descriptive one in the Repository — so say "dsp-cli's Project" when the VRE record is meant.
- **"Resource"** means two things: dsp-cli's Resource (an instance of a VRE resource-type, identified by an IRI and optionally an ARK; `dsp-cli/CONTEXT.md`) and the Archive Area's `Resource` entity (a preservation-grade unit paired with one or more Representations, versioned and ARK-identified). _Avoid_: "resource" unqualified where both a live VRE instance and an archived unit could plausibly be meant. Resolution: "VRE resource" for dsp-cli's, "archived Resource" for the Archive Area's; each context's own file is unqualified internally.
- **"Field"** and **"Value"** mean two things each: dsp-cli's Field (a slot defined on a VRE resource-type, e.g. `title`) and Value (the data held in a field on a specific VRE resource; `dsp-cli/CONTEXT.md`), versus the editor's `registry::FIELDS` (the statically registered form-field definitions that drive the depositor's UI) and the values a depositor enters into those form fields. _Avoid_: "field" or "value" unqualified in prose that spans both a VRE read and an editor form. Resolution: "dsp-cli field/value" for the VRE data model's, "form field" or "registry field" for the editor's.
- **"Vocabulary"** is dsp-cli's word for a project-scoped, hierarchical controlled vocabulary of named, labelled nodes (`dsp-cli/CONTEXT.md`) — what DSP-API itself calls a "list". _Avoid_: "list" (DSP-API's own term; it also collides with dsp-cli's own `list` verb, used across every other noun-group). Resolution: "dsp-cli's vocabulary" or "a DSP-API list" for dsp-cli's tree entity; "closed vocabulary" for the editor's and the shared metadata contract's competing sense — a field's admitted value set (`typeOfData`, in both `shared-metadata`'s contract and the editor's registry), with no tree and no DSP-API entity behind it.
