...
Enforced by: today, none (**docs-only**), with the "Divergence from the target architecture" table in `docs/src/editor/architecture.md` as the record of what is still unenforced. Once the refactor lands: the two database files, each opened by one store crate (**structure**); `.github/scripts/check-capability-deps.sh` over `cargo metadata`, run by `just check` (**static-analysis**); Bazel `visibility` after ADR-0001 (**structure**).
