...
component: mosaic/playground
module: mosaic/playground
...
The Mosaic design token infrastructure defines `font-display` (Lora) and `font-body` (Lato) as typography tokens. The playground app loads these fonts from Google Fonts via `<link>` tags in `playground/src/app.rs`.
...
- The playground app is an internal documentation tool, not a public-facing product.
...
- The playground becomes publicly deployed or handles sensitive data.
