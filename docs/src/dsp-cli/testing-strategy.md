...
action-level tests with mocks and snapshotted output. Live tests against a real DSP environment
are opt-in and not in CI.
...
5. **Live tests** — `tests/live_*.rs`, behind the `live` cargo feature; not in CI. Hit an existing
   DSP environment (developer-selected via env vars). Smoke-check end-to-end integration before
   significant changes.
