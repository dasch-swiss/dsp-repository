...
That channel is **not enforced, deliberately**. The editor never generates such a URL — both login forms are `method="post"`, which a test asserts — but a hand-typed or shared `/login?email=someone@example.org` would be exported. The obvious guard is a middleware stripping query strings from the login routes, and it was rejected: the next issue adds `/login?next=…` for redirect-preserving sign-in, so the guard would be built and dismantled in consecutive changes. Whoever adds the first query-bearing route owns this: either keep addresses out of it, or suppress `url.query` for the service.
