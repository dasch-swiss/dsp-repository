...
The beacon endpoint is not editor code. `shared-telemetry` owns the wire contract *and* the collector, and both services wire the same handler:

```rust
.route(
    "/telemetry/collect",
    shared_telemetry::collector::collect_route("editor", page_url::normalize_page_url).layer(rate_limit),
)
```
...
`page_url::normalize_page_url` is not shared code either: a shared crate cannot hold one service's route table, so the editor owns its own `page_url.rs` (`server/src/page_url.rs`) and passes the fn in. New full-page routes need a matching entry there — see `REVIEW.md`.
