...
**Last paused: 2026-05-21, after Q20 resolved by decision 47 (producer-facing surface unification; Ingest Service as sole gate; gRPC as Producer-Archive interface). Conversation paused cleanly; nothing in flight. Two new parking entries created (Q21 self-service preservation frontend; Q22 SIP wire-format sub-questions).**

#### Current state (2026-05-21)

**Q20 is resolved by decision 47.** Ingest Service is the sole producer-facing surface of the Archive; gRPC is the Producer-Archive interface across all surfaces; `CommandAPI` moves to internal-only (mTLS-authenticated principals: Ingest, preservation admin tooling); Archive Storage self-defends with re-validation; SIPs are not preserved (held at Ingest for a backup window only). Decision 47 amends decision 31 (transport: HTTP → gRPC; SSE → gRPC server-streaming for `EventStream`; CommandAPI: public → internal-only). **No design thread is "in flight."** Two new parking-lot items added during the 2026-05-21 work — Q21 (self-service preservation frontend; consolidates Metadata Editor and SIP submission GUI; bounded-context status open) and Q22 (gRPC envelope shape sub-questions; external-Producer translation gateway via BagIt deferred; mixed-content SIPs). Previous parking-lot items (Q16, Q17, Q18, Q19) remain open with their original constraints.
...
4. Pick one item from the **Top priorities for next session** picker. **Recommended pick** is the DAO-shape session against a real VRE export (Q10 + Q16 combined) — Ivan explicitly flagged this as the next thread at session close.
5. If picking a research-needed item (Q16 sub-questions / Q17 / Q19 / Q22 sub-questions), dispatch the research before architectural commitment per §9.5.

#### Top priorities for next session — choose one

With Q20 resolved by decision 47, no single design thread is "in flight." Ivan flagged the next thread explicitly at session close: **DAO-shape from VRE export (Q10 + Q16 combined)**. Listed by urgency:

| Kind | Item | Status | Rationale for priority |
|---|---|---|---|
| **Design (concrete grunt work, with real data)** ★ Recommended | **Q10 + Q16 combined — DAO-shape from VRE export** | OPEN + Parked | Walk a real VRE project export (Turtle dump + project ontology) to nail down `dao:Resource`, `dao:Representation`, and `dao:Ontology` shapes. Consolidates Q10 (Representation property list) + Q16 (project ontologies as preservation-essential). Ivan explicitly named this as the next session topic. Brings DAO close to operational completeness; closes the largest interpretability question for CTS R10. |
| **Design (no research needed)** | Q18 — Compound Resources + `kb:isPartOf` / `kb:seqnum` pair | Parked 2026-05-18 | Load-bearing for Q12.2 γ-projection (read-side propagation across `isPartOf` needs queryable structure); affects DAO SHACL shapes |
| **Design (recently parked)** | Q21 — Self-service preservation frontend | Parked 2026-05-21 | New external-facing component; bounded-context status open; expected to become active *after* DAO-shape lands (the editor manipulates whatever DAO-shape settles to) |
| **Research dispatch** | Q17 — ARK Resolver behaviour model | Parked, research-needed | Must land before CTS application (R03 succession + R12 PIDs). Five sub-dimensions |
| **Research dispatch** | Q19 — Policy ontology selection | Parked, research-needed | Closes the last sub-thread from Q12.2.2. Five candidates with prima-facie analysis already in §7 item 19 |
| **Research dispatch (deferred)** | Q22 — gRPC envelope shape; external-Producer translation gateway (BagIt drop-in) | Parked 2026-05-21 | Sub-question (a) is design grunt work, can be paired with implementation; sub-question (b) defers until external-Producer access is actually scoped (no current driver) |
| **Design (partially engaged)** | Q14 — Preservation-action workflows (response and format-migration policy) | Partial | Fixity cadence+granularity settled (decision 40); response workflow + format-migration triggering + system-ontology-migration policy still open |
| **Long-horizon** | Q13 — Project metadata as ontology | OPEN | Large workstream; defer unless a near-term project demands it |

**Recommended next move**: tackle **Q10 + Q16 combined** with a real VRE export in hand. Ivan flagged this explicitly at session close. The session needs: a representative VRE project export (Turtle / N-Triples dump from a project ideally with a custom ontology and multiple resource types), the project's ontology file(s), the relevant `knora-base.ttl` excerpts, PREMIS DD §1.2 reference (in `standards/`), and the current §4 / §6 / decision 9 amendment framings. **Output target**: `dao:Resource` property list (what `kb:*` properties survive vs. get stripped), `dao:Representation` property list + blank-node shape (decision 33), `dao:Ontology` shape (the artifact wrapping a preserved project ontology — Q16's option (a) `dao:hasOntology` on `dao:Project` is the prima facie shape). **Alternative ordering**: pick by certification urgency (Q17 ARK Resolver research first, since R03 + R12 are CTS-application gating).
