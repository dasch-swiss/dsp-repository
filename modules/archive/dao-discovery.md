...
**Last paused: 2026-05-18, after Q12 fully resolved (four decisions: Q12.1 + decisions 44 / 45 / 46). Conversation paused cleanly; nothing in flight.**

#### Current state (2026-05-18)

**Q12 is fully resolved.** Four sub-resolutions: Q12.1 (γ delivery-layer enforcement, 2026-05-15); Q12.2.1 (decision 44 — explicit per-Version `dao:accessRights`, no propagation in DAO); Q12.2.2 (decision 45 — `dao:AccessPolicy` as first-class entity with opaque policy content); Q12.2.3 (decision 46 — DAO carries policy lifecycle, γ-per-access logs are operational). **No design thread is "in flight."** Four parking-lot items were added during the 2026-05-18 work — Q16 (project ontologies preservation), Q17 (ARK Resolver behaviour model), Q18 (compound Resources `kb:isPartOf` / `kb:seqnum` pair), Q19 (policy ontology for `dao:AccessPolicy.policyContent`). Each has its own "must land before X" constraint: Q16 + Q17 before CTS application; Q18 before γ-projection implementation; Q19 before policy implementation. Q17 + Q19 (and arguably Q16) want research dispatches before architectural commitment per §9.5.
...
4. Pick one item from the **Top priorities for next session** picker. No single thread is pre-selected — Q12 closed cleanly at end of last session, and the next session chooses its direction.
5. If picking a research-needed item (Q16 / Q17 / Q19), dispatch the research before architectural commitment per §9.5.

#### Top priorities for next session — choose one

With Q12 closed, no single design thread is "next." The remaining work splits into three kinds. Listed by urgency:

| Kind | Item | Status | Rationale for priority |
|---|---|---|---|
| **Design (no research needed)** | Q18 — Compound Resources + `kb:isPartOf` / `kb:seqnum` pair | Parked 2026-05-18 | Load-bearing for Q12.2 γ-projection (read-side propagation across `isPartOf` needs queryable structure); affects DAO SHACL shapes; doesn't require external research |
| **Design (concrete grunt work)** | Q10 — Representation property list | OPEN | Walk PREMIS DD §1.2 + Knora `:FileValue` properties; produce canonical list; specify blank-node shape (decision 33). Brings DAO closer to operational completeness |
| **Research dispatch** | Q17 — ARK Resolver behaviour model | Parked, research-needed | Must land before CTS application (R03 succession + R12 PIDs). Five sub-dimensions. Per §9.5 "research before architectural commitment" |
...
| **Research dispatch** | Q16 — Project ontologies as preservation-essential metadata | Parked, research-needed | Must land before CTS application (R10 interpretability). Three options + audit-anchor framing already in §7 item 16 |
...
**Recommended next move**: tackle **Q18** next (concrete design work, downstream of Q12.2, doesn't need research), and **dispatch Q16 / Q17 / Q19 as parallel research threads** to be ready when their design turn comes. The research dispatches are the §9.5 pattern that produced decisions 33-39 (Fedora 6 + LDP research; OCFL-as-event-store research) — they pay back well. Alternative ordering: pick by certification urgency (Q16 first, then Q17 — both CTS-application gating).
