...
**On-disk RDF serialisation: N-Triples** (decision 34). All DAO RDF at rest in OCFL is serialised as N-Triples — event payloads stored as RDF, Representation `rep.nt`, etc. The Commands and Events APIs accept and emit JSON-LD or Turtle for content negotiation; normalisation to N-Triples happens at the API boundary, on write.
...
6. **Q14 Preservation-action workflows (policy)** — OPEN. `dao:PreservationAction` class is settled (decision 26). What's open: orchestration policy — who triggers, who reviews, how failures escalate.
...
11. **Q8 CoreTrustSeal evidence linkage** — PARTIALLY ENGAGED (decision 39 names HMAC+HSM and Merkle Tree log structure as the concrete deferred non-repudiation approaches, with explicit threat-model boundary). Full annotation pass across the doc still pending — add a `CTS:` callout next to each decision that produces audit evidence; cross-reference R07 / R09 / R10 / R13 / R14 / R16.
...
15. **§3.6 / §3.7 narrative prose rewrite** — OPEN. The decision log now reflects decisions 36-39 and the URN amendment, but the storage prose sections (§3.6 "Bitstream storage", §3.7 "Storage architecture", §3.7.1 "Public APIs", §3.7.2 "Subscriber bootstrap") still describe the older single-OCFL-store model with HTTPS URIs and per-Deposition/per-PreservationAction Objects bundling events with bytes. The decisions are the source of truth; the prose needs to catch up. **Mechanical doc work**, no further design decisions needed; can be done in any future session.
...
The 2026-05-14/15 session ended with the storage-architecture rewrite *decided* (decisions 36-39 + decision 12 amendment) but with two open work threads. Pick up here:

**Top priority — Q12 access-rights composition (PAUSED mid-flight).** Q12.1 resolved (γ delivery-layer enforcement). Q12.2 was in flight when we pivoted to the storage research. With decisions 33 (no `dao:File` class; per-File info as `dao:hasFile` blank nodes on Representation) and 36 (two-substrate storage) now settled, Q12.2 can resume on a stable foundation. Specific sub-questions still open:

1. **Multi-level composition** — confirm IE / Representation / File-as-blank-node-on-Rep as the three levels at which `dao:AccessRights` applies, with most-restrictive-wins composition and no "exception up the chain". (Was outlined; needs final confirmation now that the storage model is stable.)
2. **Structure of the `restricted_access` policy** — is it a simple property value with a URI reference to a separate policy document, or does it require a `dao:AccessPolicy` class with structured rule fields? (COAR's `restricted_access` is the only non-trivial case; the other three values work as simple properties.)
3. **Audit trail mechanism** — restricted-access decisions are made at the delivery layer (γ). Where does the audit trail of "who accessed what when" live? It's not a DAO event (that's the Archive's domain); it's a delivery-context concern. But the policy that *enabled* the access is in DAO. Likely answer: delivery-layer logs are operational; the Archive carries the policy and reflects access-policy *changes* as `AccessRuleChanged` events (which already exists in §5.1).

**Second priority — §3.6 / §3.7 prose rewrite (mechanical doc work).** The decision log captures decisions 36-39 + decision 12 URN amendment, but the narrative sections describing storage still match the older single-OCFL-store model. No further design decisions are needed; the rewrite is mechanical. Could be done in parallel with Q12 or as a separate session. Touch-points:
- §3.6 "Bitstream storage" — currently describes content-addressed bytes inside event payloads; rewrite to describe Preservation File bytes inside Representation OCFL Objects + the bytes-index in `cache/`.
- §3.7 "Storage architecture" — currently describes the old single-OCFL-store directory tree; rewrite to describe the two-substrate model (event-log storage root + entity storage root + active log + cache).
- §3.7.1 "Public APIs of the Archive" — mostly compatible with the new model; touch up references to URN scheme (decision 12 amendment) and clarify Binary retrieval API uses the bytes-index internally.
- §3.7.2 "Subscriber bootstrap" — mostly compatible; touch up the α/γ/δ descriptions to mention how snapshots interact with the entity-storage-root (each OCFL Object version IS a snapshot of that aggregate; subscriber-side snapshots are their own derived state, separate).

**Third priority — `CONTEXT.md` and `UBIQUITOUS_LANGUAGE.md` touch-ups.** Both reference HTTPS URIs in their Internal IRI entries; need updating to the URN scheme (decision 12 amendment). Small surgical edits, no narrative changes.
