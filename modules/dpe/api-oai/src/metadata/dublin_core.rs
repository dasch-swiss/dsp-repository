...
pub fn project_to_dublin_core(project: &Project, lookup: &dyn ContributorLookup) -> DublinCoreRecord {
    ...
    let title = if !shared_metadata::is_placeholder(&project.official_name) && !project.official_name.is_empty() {
    ...
    if !shared_metadata::is_placeholder(&project.start_date) && !project.start_date.is_empty() {
    ...
    for legal in &project.legal_info {
        if !shared_metadata::is_placeholder(&legal.license.license_uri) && !legal.license.license_uri.is_empty() {
