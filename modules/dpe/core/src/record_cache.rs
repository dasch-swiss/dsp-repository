...
fn load_all_records() -> Vec<Record> {
    ...
    let Ok(entries) = fs::read_dir(&records_dir) else {
        eprintln!("record_cache: failed to read {:?}", records_dir);
        return Vec::new();
    };
    ...
    for entry in entries.flatten() {
        ...
        match fs::read_to_string(&path) {
            Ok(json) => match serde_json::from_str::<Vec<Record>>(&json) {
                ...
                Err(e) => eprintln!("record_cache: failed to parse {}: {}", filename, e),
            },
            Err(e) => eprintln!("record_cache: failed to read {}: {}", filename, e),
        }
    }
    records
}
