...
pub(crate) async fn project_details_handler(
    ...
    let (status, view) = match project_metadata {
        None => (StatusCode::NOT_FOUND, api::routes::project::get_not_found_page()),
        Some(pm) => (
            StatusCode::OK,
            api::routes::project::get_project_details_page(pm.research_project),
        ),
    };
    Ok((status, Html(view)))
}
