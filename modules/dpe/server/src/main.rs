...
async fn project_page_handler(
    ...
    let tp = traceparent::extract_traceparent();
    let active_tab = tab.tab.as_deref().unwrap_or("overview");
...
/// 404 fallback (after `ServeDir` finds no matching static file). Reproduces the
/// previous Leptos router fallback: the shell with a "Page not found." body.
async fn not_found(
    axum::extract::State(state): axum::extract::State<AppState>,
...
async fn serve() -> ExitCode {
    ...
    // Static assets + 404, replacing leptos_axum::file_and_error_handler: serve
    // files from the public dir, falling back to the "Page not found." shell.
    ...
    let app = Router::new()
        ...
        // Page routes (replace leptos_routes + generate_route_list).
        ...
        // Datastar SSE + JSON endpoints (unchanged).
        ...
        // Static assets + 404 fallback (replaces file_and_error_handler).
