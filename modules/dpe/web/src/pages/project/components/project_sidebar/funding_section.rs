                    ...
                    div class="space-y-2" {
                        ...
                        @for grant in grants {
                            ({
                                info_card(
                                    html! {
                                        div { @ for (i, funder_id) in grant.funders.iter()
                                        .enumerate() { span { @ if i > 0 { ", " }
                                        (organization_name(funder_id)) } } } @ if let Some(number) =
                                        & grant.number { div { "Grant: "(number) } } @ if let
                                        Some(name) = & grant.name { div { (name) } } @ if let
                                        Some(url) = & grant.url { a href = (url) class =
                                        "text-primary items-center gap-1" target = "_blank" {
