    ...
    html! {
        div {
            ...
            div class="space-y-2 text-sm" {
                @for pub_ in publications { (publication_card(pub_)) }
            }
        }
    }
}

/// One publication in an info card: its citation text plus, when present, a
/// link to its persistent identifier.
fn publication_card(pub_: &Publication) -> Markup {
    let body = html! {
