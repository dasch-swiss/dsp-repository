...
//!
//! The body is a list of pairs, `Form<Vec<(String, String)>>`, not a struct:
//! `axum::Form` deserializes with `serde_urlencoded`, which errors on a repeated
...
//! field registry's job in `editor-web`, so a field's control and its decoder
//! are declared together and cannot drift.
//!
//! Absent is not empty. A name missing from the body means "this section did
//! not carry that field" and every applier leaves it alone; a name present with
//! an empty value means "the depositor cleared it". A section only posts its own
//! fields, so treating absent as cleared would have saving one section wipe
//! every other. Two HTML shapes submit nothing when empty and need a same-named
//! hidden marker beside them: an unchecked checkbox group, and a repeatable list
//! with no rows.
//!
//! Saving without changing anything must change nothing. A native submit posts
//! newlines as CRLF and Datastar's `FormData` path posts LF, so
//! `normalise_newlines` settles the comparison; a value differing from the
//! stored one only in surrounding whitespace keeps the stored bytes; a stored
//! placeholder renders as an empty control and survives an empty submit. All
//! three are pinned by `editor-web/tests/untouched_form_round_trip.rs`, which
//! runs the form over the committed corpus.
...
///
/// Both arrive inside a field name the server uses to build map keys, so an
/// unbounded one is a way to put arbitrary bulk into a stored draft. Twelve
/// holds every tag in the data with room for a subtag, and every key this
/// service mints.
const MAX_NAME_SEGMENT: usize = 12;

/// The most values one field may carry, whatever shape it is: language tags
/// under a prefix, or repeated values under one name.
///
/// One number, deliberately: it is both the bound on work (`DraftMultilingual`
/// is an order-preserving `Vec` whose `get` and `set` scan) and the cap a
/// depositor is shown. A visible cap set above the work bound would silently
/// drop every value between the two. The route refuses a body over it
/// ([`FormBody::exceeds_entries`]) before any applier runs; the truncation in
/// [`FormBody::entries`] is a floor for callers that do not check first.
/// `submit::tests::the_cap_is_above_anything_the_committed_corpus_holds` keeps
/// it clear of the corpus.
...
///
/// Every reader here is linear in the number of pairs, and must stay so:
/// nothing bounds how many arrive under one name but Axum's 2 MB body limit.
/// Hence the `HashSet` beside each de-duplicating result, and [`Self::entries`]
/// returning values with their suffixes rather than leaving the caller to fetch
/// each with [`Self::get`]. A product cap (at most so many keywords) is a
/// different thing and belongs with the route.
...
impl FormBody {
    ...
    ///
    /// The `{field}.row` marker survives removing the last row: with no marker
    /// an applier reads the field as absent and leaves the stored value alone,
    /// so the removal would not stick. Here rather than in a handler because
    /// both write surfaces need it.
    #[must_use]
    ...
    /// First rather than last: no browser sends a scalar field twice, so a repeat
    /// is our own markup or a hand-built request, and picking deterministically
    /// is enough for both.
    #[must_use]
    ...
    /// Every value posted under `name`, in body order. The name is copied into
    /// the closure so a caller can pass one built on the spot.
    ...
    /// The row keys of a repeatable field, in DOM order, as the `{field}.row`
    ...
    /// by index then point at the wrong rows. Keys that are empty, over-long or
    /// not `[A-Za-z0-9_-]+` are dropped, since a row key becomes part of a field
    /// name the server reads back; duplicates are dropped too, keeping the first,
    /// because two rows with one key collapse wherever the key is a map key.
    #[must_use]
    ...
    /// Whether more than `cap` values arrived under `name` itself: the
    /// repeated-name counterpart of [`Self::exceeds_entries`]. Counts every pair
    /// rather than distinct values, because a body carrying a thousand copies of
    /// one value is the same amount of work to read.
    #[must_use]
    ...
    ///
    /// Cannot be done with [`Self::entries`], which stops at
    /// [`MAX_VALUES_PER_FIELD`] and so cannot tell twenty thousand tags from
    /// sixty-four. Stops as soon as the answer is known, so the work stays linear
    /// in the pairs and bounded by `cap` in memory.
    #[must_use]
    ...
    /// without duplicates, at most [`MAX_VALUES_PER_FIELD`] of them.
    ///
    ...
    /// that knows which are present. A suffix containing a further `.` is a
    /// deeper path this shape does not own and is skipped. The value comes back
    /// with the suffix in one pass, because fetching each with [`Self::get`] is
    /// quadratic in the pairs. First occurrence wins, matching [`Self::get`].
    #[must_use]
...
/// pass, and a hand-built body could put a junk key into a stored language map,
/// from where it rides out into a published file.
...
///
/// A native submit posts a newline as CRLF (the urlencoded serializer normalises
/// them) while `new FormData(form)`, Datastar's path, preserves LF; without this
/// the two paths write different bytes for the same untouched value. A lone CR
/// is normalised too, since leaving it would put a bare control character in
/// published JSON.
...
pub enum WhenCleared {
    ...
    /// the committed data spells that way, such as `endDate` on an ongoing
    /// project. Dropping it instead would make every ongoing project
    /// unpublishable. Which field gets which is declared once, on
    /// `editor_web::form::registry`'s `Field`, as part of its [`Shape`].
    Placeholder,
}
...
///
/// A closed set is the wrong answer for most of these fields: closing
/// `dataLanguage` to the four tags the UI offers would drop `la` from committed
/// projects on their first save, because a value with no control posts nothing.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ChoiceSet {
    /// Only these values are stored; anything else is dropped.
    Closed(&'static [&'static str]),
    /// These are what the form offers, and any other value is kept. The slice is
    /// the offer, not the accepted set: the widget unions it with whatever the
    /// project already holds, so an unusual tag keeps its control.
...
///
/// One arm per applier, so naming a shape is the only way to reach one, and the
/// field registry gives every field exactly one. [`WhenCleared`] rides inside
/// [`Self::Text`] because it is meaningful for nothing else: a language map's
/// empty state is "no tags".
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Shape {
    /// A scalar string member, read by `apply_text`. The [`WhenCleared`] is the
    /// contract's own distinction between a required `String` with a sentinel
    /// empty state and an `Option<String>`.
    Text(WhenCleared),
    /// A language map, from `{field}.<tag>` pairs. Read by `apply_multilingual`.
    Multilingual,
    /// One of the project's two URLs, read by `apply_url`. A slot rather than a
    /// member: [`UrlSlot`] says where the pair lives and why the two fields are
    /// applied independently.
    Url(UrlSlot),
    /// A list of strings from repeated `{field}` values, read by
    /// `apply_string_list`. The [`ChoiceSet`] decides whether a value the form
    /// did not offer is kept, a contract question: `typeOfData` is a closed
    /// vocabulary, `dataLanguage` is open.
    StringList(ChoiceSet),
    /// A list of authority references, read by `apply_reference_rows`:
    /// `spatialCoverage`. Not [`Self::TextOrReferenceRows`] with an unused
    /// branch: the contract types this member as `Vec<AuthorityFileReference>`,
    /// so a free-text row would not deserialize.
    ReferenceRows(&'static [&'static str]),
    /// A list of bibliographic references, read by `apply_publication_rows`:
    /// `publications`. A row is a citation plus an optional persistent
    /// identifier; the `text` the contract allows beside the identifier is
    /// unused in the corpus and not offered.
    PublicationRows,
    /// Funding, read by `apply_funding`: either a list of grants or one free
    /// text. The discriminant is on the field, not per row, which is what makes
    /// this its own shape.
    FundingRows,
    /// A list of rows that are each either an authority reference or free text
    /// per language, read by `apply_text_or_reference_rows`: `temporalCoverage`
    /// and `disciplines`. The slice is the reference types the field offers.
    ///
    /// Both variants are live in the committed data, so an entry must never be
    /// coerced from one into the other. Narrowing consults only the discriminant
    /// the row posts, never the values beside it: serde's untagged enums would
    /// read a half-filled reference as text.
    ...
    /// `contactPoint`. Stored and posted exactly as [`Self::StringRows`] is; the
    /// difference is meaning, since the form resolves each id and submit refuses
    /// one that resolves to nothing. A variant so the submit check reads it off
    /// the shape rather than off a second list.
    AgentRows,
    /// A list of single strings as editable rows, read by `apply_string_rows`:
    /// `additionalMaterial` and `documentationMaterial`. Rows rather than a
    /// checkbox group because these hold URLs, which have to be editable.
    StringRows,
    /// A list of language maps, read by `apply_multilingual_rows`: `keywords`
    /// and `alternativeNames`. Rows come from `{field}.row` in DOM order and each
    /// row's texts from `{field}.<key>.<tag>`.
    MultilingualRows,
...
impl Shape {
    /// Whether a field of this shape renders a control that refers to a person or
    /// organisation. Exhaustive on purpose, so a new shape cannot be added without
    /// answering the question; it gates the proposals summary.
    #[must_use]
    ...
    /// Whether a field of this shape renders as a list of rows, and therefore
    /// carries the add and remove controls the row-action routes serve.
    ///
    /// Exhaustive for the same reason [`Self::has_agent_picker`] is: a shape
    /// missing from a hand-written allowlist renders an add control whose
    /// `formaction` answers `404` and discards the whole form, and nothing
...
/// Apply one field, whichever shape it is. The only door out of this crate (the
/// appliers are `pub(crate)`), so a handler cannot pick a `WhenCleared` the
/// registry does not declare; `Drop` on a required `String` leaves every ongoing
/// project unpublishable with nothing failing.
...
/// and one `{field}.<key>` value each: the row protocol [`apply_multilingual_rows`]
/// reads, with one text per row.
///
/// An empty row is dropped, since an added-but-unfilled row is an editing state.
/// Absent `{field}.row` leaves the field alone; present with no valid key, the
/// marker an empty list posts, clears it. A text differing from a stored one
/// only in whitespace keeps the stored bytes, matched across the whole list for
/// the reason [`unchanged_in`] gives.
...
///
/// Absent from the body leaves the slot alone. An empty value clears it, since
/// both members are `Option`, except against a stored placeholder, which is
/// already the empty state and renders as an empty control: writing `""` over
/// `url: ["MISSING"]` would change a field nobody edited. The write goes through
/// [`ProjectDraft::set_url_slot`], which preserves the stored representation.
...
///
/// Absent leaves the field alone, and so does an empty or unrecognised value,
/// which is the difference from [`apply_text`]: these are required members
/// typed as enums with no unset variant, and storing one would produce a draft
/// that cannot become a `ProjectRaw`. A value outside `values` can only arrive
/// from a hand-built body, and dropping is the fail-safe direction.
pub(crate) fn apply_choice(body: &FormBody, draft: &mut ProjectDraft, field: &str, values: &[&str]) {
    ...
    // No "only write if it changed" guard: the untouched round-trip test is the
    // only path that exercises this applier, and a guard would make the write
    // unreachable there, so a corrupted applier would pass.
    draft.set(field, Value::String(submitted.to_string()));
}

/// Apply a submitted scalar to `field`.
///
/// Absent from the body leaves the field alone. The value is trimmed. A stored
/// placeholder survives an empty submit: `MISSING` and `CALCULATED` are the
/// platform's "no value yet" sentinels, so a control holding one renders empty
/// and an untouched form posts an empty value for it; writing `""` back would
/// change a published field nobody edited.
...
///
/// The tags come from the body rather than a fixed list, so a tag outside the
/// four the UI offers survives a save. An empty text drops its tag, as
/// [`DraftMultilingual::to_contract`] does, so `"en": ""` never reaches a file
/// where DPE's language fallback would render a blank. A map left entirely empty
/// removes the field.
pub(crate) fn apply_multilingual(body: &FormBody, draft: &mut ProjectDraft, field: &str) {
    ...
    for (tag, text) in posted {
        // Folded, because the tag becomes a key in the stored map and the
        // published file, and BCP 47 treats `EN` and `en` as one language. Two
        // case variants collapse to one entry, body order deciding which wins.
        let tag = &tag.to_ascii_lowercase();
        let stored_text = stored.get(tag);
        match resolve(text, stored_text) {
            // Only non-empty texts reach the value: `DraftMultilingual::set` keeps
            // an empty one for the editing view, and a map of empties would be
            // stored as `{}`.
            Resolved::Cleared => {}
            // `Unchanged` is only returned when there is a stored text; written so
            // that a fall-through keeps what the tag had.
            Resolved::Unchanged => {
...
/// the shape a checkbox group and a plain repeated input both post.
///
/// Empty values are dropped, which is what makes the hidden marker work: a group
/// with nothing checked posts only the marker, so the field arrives present and
/// empty and is cleared. Duplicates are dropped, keeping the first. A value
/// outside a [`ChoiceSet::Closed`] set is dropped for the reason [`apply_choice`]
/// drops one.
...
/// whitespace or line endings, if any row holds one: the row-list counterpart
/// of the `stored` argument to [`resolve`]. Returns the first match.
...
/// than written as `{}`. Absent `{field}.row` leaves the field alone; present
/// with no valid key, the hidden marker of an empty list, clears it.
pub(crate) fn apply_multilingual_rows(body: &FormBody, draft: &mut ProjectDraft, field: &str) {
    ...
    for key in body.rows(field) {
        ...
        for (tag, text) in body.entries(&prefix) {
            ...
            let tag = tag.to_ascii_lowercase();
            // Looked up by text across every row, not by position: a row key is
...
///
/// The list counterpart of [`resolve`]'s `stored` argument, searching the whole
/// list rather than one position, since row keys are opaque. An exact match is
...
/// `{field}.<key>.contributor` and its roles from repeated `{field}.<key>.role`
/// values, so a checkbox group and an "add another" input post into one list.
///
/// A row is defined by its contributor: one with no id is dropped. An empty role
/// list is kept, since the contract types `contributorType` as a `Vec<String>`
/// and a contributor whose role nobody has stated yet is expressible. Roles keep
/// their stored bytes through [`resolve_against`]: committed values end in a
/// space and spell the same role several ways, so neither trimming nor
/// case-folding may touch what is there.
pub(crate) fn apply_attribution_rows(body: &FormBody, draft: &mut ProjectDraft, field: &str) {
    ...
    for key in body.rows(field) {
        ...
        let mut row = Map::new();
        // `Attribution`'s declaration order, so a draft already matches what the
...
/// and free text at `{field}.<key>.text.<tag>`. The text branch is namespaced
/// under `.text.` because [`FormBody::entries`] reads every dotted suffix under
/// a prefix as a language tag.
///
/// Only the discriminant narrows. The inactive branch is rendered `hidden`, not
/// `disabled`, so both candidates arrive on every save; reading the values
/// instead is how a half-filled reference gets silently stored as text.
...
/// Every string this field already holds, at any depth: what [`resolve_against`]
/// needs, flattened rather than walked per row because row keys are opaque.
...
///
/// A row is its citation: one with no text is dropped. The identifier is
/// optional and its member is omitted entirely when absent, never written as
/// an object with an empty URL.
...
///
/// The discriminant is on the field rather than per row, so there is one
/// `{field}.kind` for the whole control; both branches are submitted and only
/// the discriminant decides, as in [`apply_text_or_reference_rows`]. A grant is
/// its funders, and one with none is dropped. `number`, `name` and `url` are
/// omitted when empty rather than written as `""`, as the committed data holds
/// them.
...
mod tests {
    ...
    fn entries_stops_at_the_cap_so_one_body_cannot_buy_unbounded_work() {
        // A 2 MB body carries on the order of 100,000 short pairs, so the cap is
    ...
    /// The closed vocabulary `typeOfData` declares, spelled out so a change to the
    /// real vocabulary cannot quietly change what these tests assert.
    ...
    fn a_draft_nobody_edited_survives_a_full_round_trip_unchanged() {
        // Every applier has an "absent is not empty" branch and a placeholder
        // branch, and this is where all of them have to agree.
        let original = draft();
    ...
    fn only_the_discriminant_decides_which_variant_a_row_becomes() {
        // A fully filled reference beside filled text: the discriminant is the
        // only difference between the two expectations.
        let filled = &[
    ...
    fn a_publication_row_is_its_citation_and_its_identifier_is_optional() {
        // The identifier's member is omitted when absent rather than written with an
        // empty URL, as the committed data does.
        let mut draft = draft();
    ...
    fn a_grant_is_its_funders_and_its_empty_members_are_omitted() {
        // A grant's optional members are absent in the committed data when unset,
        // never `""`.
        let mut draft = draft();
        apply_funding(
            &body(&[
