...
-- as a JSON object keyed by project member name. Nullable, and null
...
-- depositor has to be shown: a depositor's submission needs no second
-- approver, so the value RDU put in place of theirs is seen by nobody unless
-- the submitted one survives beside it.
--
-- Opaque to this layer, like `payload` — the reviewing handler parses it.
ALTER TABLE submissions ADD COLUMN review_state TEXT;

-- The note RDU left when it returned the project to the depositor.
--
