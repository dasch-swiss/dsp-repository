...
/// Pass `roles` to display them between the name and job titles.
#[component]
        ...
        <Suspense>
            {move || {
                ...
                match person_opt {
                    Some(person) => {
                        view! {
                            <PersonView person=person roles=roles.clone() show_email=show_email />
                        }
                            .into_any()
                    }
                    None => {
...
#[component]
fn PersonView(person: PersonData, roles: Option<String>, #[prop(default = false)] show_email: bool) -> impl IntoView {
