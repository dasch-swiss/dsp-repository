...
mod tests {
    ...
    fn incunabula_project() -> Project {
        Project {
            ...
            short_description: "An art-scientific monograph of the richly illustrated early prints in Basel."
                .to_string(),
            description: {
    ...
    // ---- from/until ----
    // incunabula_project() has start_date = "2008-06-01"

    fn minimal_project(shortcode: &str, start_date: &str) -> Project {
        Project {
    ...
    fn make_date_params(from: Option<&str>, until: Option<&str>) -> OaiParams {
        OaiParams {
    ...
    #[test]
    fn date_filtering_from_until() {
        let repo = InMemoryProjectRepository::new(vec![minimal_project("0803", "2008-06-01")]);
        let no_records = InMemoryRecordRepository::empty();

        let xml = handle_list_records(&make_date_params(None, Some("2007-12-31")), &repo, &no_records);
        assert!(xml.contains("<error code=\"noRecordsMatch\">"), "until before start should exclude; got: {xml}");

        let xml = handle_list_records(&make_date_params(Some("2009-01-01"), None), &repo, &no_records);
        assert!(xml.contains("<error code=\"noRecordsMatch\">"), "from after start should exclude; got: {xml}");

        let xml = handle_list_records(&make_date_params(None, Some("2008-06-01")), &repo, &no_records);
        assert!(xml.contains("<ListRecords>"), "until on start should include; got: {xml}");

        let xml = handle_list_records(&make_date_params(Some("2008-06-01"), None), &repo, &no_records);
        assert!(xml.contains("<ListRecords>"), "from on start should include; got: {xml}");

        let xml = handle_list_records(&make_date_params(Some("2005-01-01"), Some("2020-01-01")), &repo, &no_records);
        assert!(xml.contains("<ListRecords>"), "start within from/until range should include; got: {xml}");

        let repo2 = InMemoryProjectRepository::new(vec![
            minimal_project("0803", "2008-06-01"),
            minimal_project("0804", "2015-03-01"),
        ]);
        let xml = handle_list_records(&make_date_params(None, Some("2010-01-01")), &repo2, &no_records);
        assert_eq!(xml.matches("<record>").count(), 1, "only one project within range; got: {xml}");

        let xml = handle_list_records(&make_date_params(None, None), &repo2, &no_records);
        assert_eq!(xml.matches("<record>").count(), 2, "both projects when no filter; got: {xml}");
    }
    ...
    fn record_set_filter_returns_only_records() {
        ...
        let xml = handle_list_records(&params, &project_repo, &record_repo);
        assert!(xml.contains("record-0001"), "should contain record identifier; got: {xml}");
        assert!(!xml.contains("oai:meta.dasch.swiss:ark:/72163/1/0803"), "should not contain project; got: {xml}");
    }
