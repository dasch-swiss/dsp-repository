...
use mosaic_tiles::badge::{badge, BadgeProps, BadgeSize, BadgeVariant};
...
fn b(variant: BadgeVariant, content: Markup) -> Markup {
    badge(BadgeProps { variant, ..Default::default() }, content)
}

/// Shorthand: a badge with explicit variant and size.
fn bs(variant: BadgeVariant, size: BadgeSize, content: Markup) -> Markup {
    badge(BadgeProps { variant, size }, content)
}
