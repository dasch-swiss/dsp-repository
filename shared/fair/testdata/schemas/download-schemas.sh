...
#
# Takes the directory holding the committed kernel-4.6 XSDs as its one argument,
# rather than reaching for it: a shared crate must not know a service module's
# layout (see shared/README.md). The repo-root justfile is what knows it, so
# refresh the local copy with
#
#   just refresh-datacite-schema
#
# which passes that directory. Run from anywhere; the output lands beside this
# script.
#
# Source:
...
#                             OAI XSD copies use (see the sibling
#                             download-schemas.sh beside dpe-api-oai's handler
#                             testdata).
#
...
#
# Both enum widenings read their values out of the kernel-4.6 XSDs already
# committed for the OAI tests, which is what the argument points at, so the two
# copies of the kernel's vocabulary cannot drift.
#
...
# (see `datacite_json_validator` in dpe-api-oai's corpus test).

set -euo pipefail

usage() {
  echo "usage: download-schemas.sh <xsd-include-dir>" >&2
  echo "  <xsd-include-dir>  the directory holding the committed kernel-4.6 XSDs" >&2
  echo "                     (datacite-resourceType-v4.xsd, datacite-dateType-v4.xsd)." >&2
  echo "  Prefer \`just refresh-datacite-schema\`, which passes it." >&2
  exit 2
}

[ "$#" -eq 1 ] || usage
# Resolved before the cd below, so a relative argument is read from the caller's
# working directory and not from this one.
XSD_INCLUDE="$(cd "$1" 2>/dev/null && pwd)" || usage

# The patched schema belongs beside this script, whatever the caller's cwd.
